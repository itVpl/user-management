...
const PaymentRemarkTest = () => {
  ...
  const handleTest = async () => {
    ...
    try {
      ...
      const response = await apiService.updatePaymentRemark(testDoId, testStatus, testNotes);
      setResult(response);
    } catch (err) {
      setError(err.message || 'Failed to update payment remark');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="p-6 max-w-2xl mx-auto">
      <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-6">
        <h2 className="text-xl font-bold text-gray-800 mb-6">Payment Remark API Test</h2>
        ...
        <div className="space-y-4">
          ...
          >
            {loading ? 'Testing...' : 'Test Payment Remark API'}
          </button>
        </div>
...
export default PaymentRemarkTest;
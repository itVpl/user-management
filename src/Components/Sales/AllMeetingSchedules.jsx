          ...
          <div className="flex flex-col gap-1 w-full xl:w-[350px]">
            <div className="relative w-full">
              ...
              <input
                ...
                className="w-full px-4 h-[40px] border border-gray-200 rounded-lg bg-white text-gray-700 focus:ring-2 focus:ring-blue-100 focus:border-blue-300"
              />
            </div>
            <div className="relative w-full">
                ...
                <tbody>
                  {currentMeetings.map((m) => (
                    <tr key={m._id} className="border-b border-gray-100 hover:bg-gray-50">
                      <td className="py-4 px-4 text-sm text-gray-800">{getEmployeeName(m)}</td>
      ...
      {showViewModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40" onClick={() => { setShowViewModal(false); setViewMeetingDetail(null); setSelectedMeeting(null); }}>
          <div className="bg-white rounded-2xl shadow-xl max-w-md w-full p-6" onClick={(e) => e.stopPropagation()}>
            ...
            {(() => {
              ...
              return (
                <div className="space-y-3 text-sm">
                  <p><span className="font-medium text-gray-600">Employee:</span> {getEmployeeName(m)}</p>
            ...
            <div className="mt-6">
              <button type="button" onClick={() => { setShowViewModal(false); setViewMeetingDetail(null); setSelectedMeeting(null); }} className="w-full px-4 py-2 bg-gray-100 rounded-lg hover:bg-gray-200">Close</button>
            </div>
          </div>
        </div>
      )}

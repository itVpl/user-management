...
const IddCallingReport = () => {
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "IDD Call Records");
    const fileName = `IDDCallRecords_${selectedDate}.xlsx`;
  ...
  const fetchData = async (alias, date) => {
    ...
    // Format for Triton API: YYYY-MM-DD+HH%3AMM%3ASS
    const formatForTriton = (date) => {
      const d = new Date(date);
      const yyyy = d.getFullYear();
      const mm = String(d.getMonth() + 1).padStart(2, "0");
      const dd = String(d.getDate()).padStart(2, "0");
    ...
    const from = formatForTriton(start);
    const to = formatForTriton(end);
    ...
    try {
      ...
      console.error("❌ API Error fetching filtered IDD calls:", err);
        ...
        <div className="flex items-center gap-6">
          <div className="bg-white rounded-2xl shadow-xl p-4 border border-gray-100">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm text-gray-600">Total IDD Calls</p>
              ...
              {currentRecords.length > 0 ? (
                ...
                <tr>
                  <td colSpan={6} className="text-center py-12">
                    ...
                    <p className="text-gray-500 text-lg">No IDD call records found</p>
      ...
      {totalPages > 1 && records.length > 0 && (
        <div className="flex justify-between items-center mt-6 bg-white rounded-2xl shadow-xl p-4 border border-gray-100">
          <div className="text-sm text-gray-600">
            Showing {startIndex + 1} to {Math.min(endIndex, records.length)} of {records.length} IDD call records
          </div>
...
export default IddCallingReport;
